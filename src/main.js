...
// Нам нужно, что бы использование мутированного селектора не порождало
// реакцию обновления.
...
Вначале происходит чтение b.val потом модификацию.
//Т.к. возникло чтение
Вопрос почему не обновлять?
Всегда ли в одном контексте, если произошло вначале чтение значения, потом
его модификация (исключать его из контекста зависимых).

При модификации значения, исключать write ноду из зависимых, потому что
мы сейчас обладаем самой актуальной информацией о её значении.
Но тогда не сработает инкремент. Поэтому всё тут норм, на текущем этапе.

У нас другая проблема.

Селектор, который обновляет сам себя и пораждает повторный запуск выражения.

Как это происходит.

Селектор перевызывает сам себя.
Т.е. это не просто чтение, а перевызов с порождением записи.

Что произошло.

Началось чтение селектора s.
  Читаю a
  Пишу в a
    Начался цикл записи
    Обновляю a
      Селектор s использует a - обновить
      Вызываю обновление селектора
      Селектор обновляется с модификацией. ??


Окей, пока сделаем для инициализации.
